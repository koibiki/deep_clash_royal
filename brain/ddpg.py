...
class DDPG(object):
    ...
    def _build_actor_critic_net(self, s_img, s_card_elixir, is_train=True, ):
        ...
        with tf.variable_scope('actor', ):
            ...
            card_value = tf.layers.conv2d(card_state, 93, 1, 1, padding="same")

            card_value = tf.squeeze(card_value, axis=[1, 2])
        ...
        with tf.variable_scope("critic", ):
            card_state_type = tf.concat([tf.squeeze(card_state, axis=[1, 2]), card_type], axis=-1)
        ...
        return card_value, loc_x, loc_y, q_card, q_loc_x, q_loc_y
    ...
    def _build_eval_target_net(self):
        ...
        with tf.variable_scope('eval_net'):
            self.card_value, self.loc_x, self.loc_y, self.q_card_eval, self.q_x_eval, self.q_y_eval = \
        ...
        with tf.variable_scope('loss'):
            ...
            accuracy = tf.metrics.accuracy(select_card, self.card_action)
        ...
        with tf.variable_scope('target_net'):
            _, _, _, self.q_card_next, self.q_x_next, self.q_y_next = \
    ...
    def choose_action(self, observation):
        ...
        if uniform <= 0.5:
            ...
            card, loc_x, loc_y = self.sess.run([self.q_card_eval, self.q_x_eval, self.q_y_eval],
                                               feed_dict={self.s_img: [observation[1]],
                                                          self.s_card_elixir: [parse_running_state(observation[2])]})
            card_index = np.argmax(card)
            print("dqn play:" + str(card_index))
            if card_index != 0:
                action = [card_index, loc_x, loc_y]
            else:
                action = [0, 0, 0]
    ...
    def learn(self):
        with self.sess.as_default():
            ...
            imgs, states, action, reward, next_imgs, next_states \
            ...
            q_card_next, q_card_eval, q_x_next, q_x_eval, q_y_next, q_y_eval = self.sess.run(
                ...
                feed_dict={self.s_img_: next_imgs,
                           ...
                           self.card_action: action[:, 0].astype(np.int32),
                           self.x_action: action[:, 1],
                           self.y_action: action[:, 2]})
            ...
            for i in range(len(q_card_target)):
                ...
                q_card_target[i] = reward[i] + self.gamma * np.max(q_card_next[i])
                if action_item[0] != 0:
                    ...
                    if action_item[0] in available_card:
                        q_x_target[i] = reward[i] + self.gamma * np.max(q_x_next[i])
                        q_y_target[i] = reward[i] + self.gamma * np.max(q_y_next[i])

            _, _, abs_errors, loss, summary = self.sess.run(
                [self._train_actor_op, self._train_critic_op, self.abs_errors, self.loss, self.merge_summary],
