...
from net.model import Actor, CriticPPO
...
class PPO(object):
	...
	def __init__(self, device):
		...
		self.critic_net = CriticPPO().to(device)
		self.buffer = []
		self.counter = 0
	...
	def select_action(self, img, env_state, card_state, actor_hidden=None):
		if actor_hidden is None:
			actor_hidden = self.actor_net.initHidden(len(img))
		...
		with torch.no_grad():
			action = self.actor_net(img, env_state, card_state, actor_hidden)
	...
	def get_value(self, img, env_state, card_state, critic_hidden=None):
		if critic_hidden is None:
			critic_hidden = self.critic_net.initHidden(len(img))
		img = torch.from_numpy(img / 255.).float().to(self.device)
		env_state = torch.from_numpy(env_state).float().to(self.device)
		card_state = torch.from_numpy(card_state).float().to(self.device)
		with torch.no_grad():
			value, critic_hidden = self.critic_net(img, env_state, card_state, critic_hidden)
